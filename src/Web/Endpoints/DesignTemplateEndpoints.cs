...
using sp26se058_3dprintshop_be.Application.DesignTemplates.Queries.GetDesignTemplatesManageCatalog;
...
public class DesignTemplateEndpoints : EndpointGroupBase
{
    public override void Map(IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/manage-catalog", ManageCatalog)
            .WithSummary("[Manager/Admin] Danh sách mẫu thiết kế kèm biến thể bán (một API)");
        ...
        group.MapDelete("/{id}/toggle-active", ToggleActive)
            .WithSummary("Publish / Unpublish mẫu thiết kế (bật/tắt IsActive)");
    ...
    public async Task<IResult> ManageCatalog([FromServices] ISender sender, [FromBody] GetDesignTemplatesManageCatalogQuery query)
    {
        try
        {
            var result = await sender.Send(query);
            return TypedResults.Ok(BaseResponseModel<IEnumerable<DesignTemplateManageCatalogItemDto>>.OkResponseModel(
                    code: ResponseCodeConstants.SUCCESS,
                    data: result.Items,
                    additionalData: new { pagination = result.Metadata },
                    message: "Truy vấn catalog quản lý sản phẩm thành công!"
                ));
        }
        catch (Exception ex)
        {
            return TypedResults.BadRequest(BaseResponseModel<string>.BadRequestResponseModel(
                    data: ex.Message,
                    message: "Truy vấn catalog quản lý sản phẩm thất bại!"
                ));
        }
    }
    ...
    public async Task<IResult> ToggleActive([FromServices] ISender sender, [FromRoute] Guid id)
    {
        try
        {
            var result = await sender.Send(new ToggleDesignTemplateActiveCommand { Id = id });
            var action = result.IsActive ? "Publish" : "Unpublish";
            return TypedResults.Ok(BaseResponseModel<DesignTemplateDTO>.OkResponseModel(
                    data: result,
                    message: $"{action} mẫu thiết kế thành công!",
                    code: ResponseCodeConstants.SUCCESS
                ));
        }
        catch (Exception ex)
        {
            return TypedResults.BadRequest(BaseResponseModel<string>.BadRequestResponseModel(
                    data: ex.Message,
                    message: "Cập nhật trạng thái mẫu thiết kế thất bại!"
                ));
        }
    }
