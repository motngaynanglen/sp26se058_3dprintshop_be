...
public class DeleteAccountCommandHandler : IRequestHandler<DeleteAccountCommand, bool>
{
    ...
    public async Task<bool> Handle(DeleteAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        // 1. Ki?m tra Username ho?c Email dã t?n t?i chua
        var account = await _context.Accounts.FirstOrDefaultAsync(a => a.Id == request.Id, cancellationToken);

        if (account == null)
        {
            throw new Exception("Tài kho?n không t?n t?i trong h? th?ng.");
        }
        if (!account.IsActive)
        {
        ...
        {
            throw new Exception("Tài kho?n dang ho?t d?ng! Hãy h?y quy?n tru?c.");
        }
